...
import re
import pandas as pd
import matplotlib            # Non-interactive Backend auswählen
matplotlib.use('Agg')        # ← ganz wichtig vor pyplot!
...
# 1) Pfad zur CSV-Datei (anpassen, falls nötig)
CSV_PATH = 'parkhaeuser_wetter_merged_allvars_mit_preisen_cleaned.csv'

# 2) Verzeichnis anlegen, in dem die Modelle, Importances, Plots und Metriken gespeichert werden
...
# 3) Datensatz einlesen (inkl. Timestamp)
data = pd.read_csv(CSV_PATH, parse_dates=['timestamp'])

# 4) Wetter-Spalten definieren (anpassen, falls nötig)
weather_cols = [
...
# 6) Safe-Dateiname-Funktion
def make_safe_filename(name: str) -> str:
    safe = re.sub(r'[^0-9A-Za-z]+', '_', name)
    return re.sub(r'_{2,}', '_', safe).strip('_')

# 7) Für jedes Parkhaus: trainieren, evaluieren, Outlier-CSV erzeugen
for garage in sorted(garage_prefixes):
    ...
    # e) RandomForest initialisieren & f) trainieren
    rf = RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)
    rf.fit(X_train, y_train)

    # g) Vorhersage & h) Bewertung
    ...
    plt.title(f'Vorhersage vs. Ist – {garage} (RandomForest)')
    plt.xlabel('Ist-Wert (Auslastung)')
    plt.ylabel('Vorhergesagt (Auslastung)')

    # Metrik-Tabelle im Plot einfügen
    textstr = (
        f"MAE: {mae:.2f}\n"
        f"MSE: {mse:.2f}\n"
        f"R²: {r2:.3f}"
